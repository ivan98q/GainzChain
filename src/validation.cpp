...
const std::string strMessageMagic = "GainzCoin Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.path, block.nBits, GenerateWorld(block.hashPrevBlock), consensusParams))
...
void ThreadScriptCheck() {
    RenameThread("gainzcoin-scriptch");
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.path, block.nBits, GenerateWorld(block.hashPrevBlock), consensusParams))
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.nBits != GetNextPathLength(pindexPrev, &block, consensusParams))
