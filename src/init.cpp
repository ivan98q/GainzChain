 ...
 * between gainzcoind, and gainzcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("gainzcoin-shutoff");
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), GAINZCOIN_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://gainzcoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("gainzcoin-loadblk");
...
/** Sanity checks
 *  Ensure that GainzCoin is running in a usable environment with all
...
{
    // Make sure only a single GainzCoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", GAINZCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if gainzcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if gainzcoin is started while in a temporary directory.\n",
